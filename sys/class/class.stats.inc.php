...
class stats extends db_connect
{
     ...
     * @throws DateInvalidTimeZoneException
     */
    public function getNewItemsCount($days, $tableName, $timeField): array
    {
        $result = [];
    ...
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE state = (:state) and ufake = 0");
    ...
    public function getModeratedAccountsPhotosCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE photoModerateAt > 0");
        $stmt->execute();

        return $number_of_rows = $stmt->fetchColumn();
    }

    public function getUnModeratedAccountsPhotosCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE photoPostModerateAt > 0");
        $stmt->execute();

        return $number_of_rows = $stmt->fetchColumn();
    }

    public function getModeratedAccountsCoversCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE coverModerateAt > 0");
        $stmt->execute();

        return $number_of_rows = $stmt->fetchColumn();
    }

    public function getUnModeratedAccountsCoversCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE coverPostModerateAt > 0");
        $stmt->execute();
    ...
    public function getAccountsCountByGhost($value)
    {
        $current_time = time();
        $ghost_create_at = $current_time - 2592000;

        if ($value == 1) {

            $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE ghost_create_at > (:ghost_create_at)");
    ...
    public function getAccounts($userId = 0)
    {
        ...
        $stmt = $this->db->prepare("SELECT id FROM users WHERE id < (:userId) AND ufake = 0 ORDER BY id DESC LIMIT 20");
