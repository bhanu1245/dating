...
class find extends db_connect
{
    ...
    private function getCount($queryText, $gender = 3, $online = 0, $photo = 0, $proMode = 0, $ageFrom = 18, $ageTo = 105, $sexOrientation = 0)
    {
        ...
        $sql = "SELECT count(*) FROM users WHERE registrationComplete = 1 AND state = 0 AND (login LIKE '{$queryText}' OR fullname LIKE '{$queryText}' OR email LIKE '{$queryText}' OR country LIKE '{$queryText}')".$genderSql.$onlineSql.$photoSql.$proModeSql.$sexOrientationSql.$dateSql;
    ...
    public function query($queryText = '', $itemId = 0, $gender = 3, $online = 0, $photo = 0, $proMode = 0, $ageFrom = 18, $ageTo = 105, $sexOrientation = 0)
    {
        ...
        $sql = "SELECT id, regtime FROM users WHERE registrationComplete = 1 AND state = 0 AND (login LIKE '{$queryText}' OR fullname LIKE '{$queryText}' OR email LIKE '{$queryText}' OR country LIKE '{$queryText}') AND id < {$itemId}".$genderSql.$onlineSql.$photoSql.$proModeSql.$sexOrientationSql.$dateSql.$endSql;
        $stmt = $this->db->prepare($sql);
    ...
    public function start($queryText = '', $itemId = 0, $gender = 3, $online = 0, $photo = 0, $proMode = 0, $ageFrom = 18, $ageTo = 105, $sexOrientation = 0, $distance = 3000, $lat = "", $lng = "")
    {
        ...
        $sql = "SELECT id, regtime, lat, lng, 3956 * 2 *
                    ...
                    *POWER(SIN(($origLng-lng)*pi()/180/2),2)))
                    as distance  FROM users WHERE registrationComplete = 1 AND state = 0 AND (login LIKE '{$queryText}' OR fullname LIKE '{$queryText}' OR email LIKE '{$queryText}' OR country LIKE '{$queryText}') AND id < {$itemId}".$genderSql.$onlineSql.$photoSql.$proModeSql.$sexOrientationSql.$dateSql.$endSql;
        $stmt = $this->db->prepare($sql);
